...
static int _klcko_process_cmd(int cmd, void *data, int data_len, void *reply, int reply_size)
{
    ...
    if (reply && reply_size) {
        r = LVM_MallocExt(&lvm, reply_size, VMEM_ZMalloc);
        if (! r) {
            return KO_ERR_NO_MEMORY;
        }
    }

    ret = func(subcmd, data, data_len, r, reply_size, &reply_len);
    if ((reply_len > 0) && r) {
        if (0 != copy_to_user(reply, r, reply_len)) {
            ret = KO_ERR_FAIL;
        }
    }

    LVM_FreeExt(&lvm, VMEM_Free);
